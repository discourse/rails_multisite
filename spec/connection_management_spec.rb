...
describe RailsMultisite::ConnectionManagement do
  ...
  context "with default" do
    ...
    it "yields self for with_connection" do
      x = conn.with_connection("default") { "hi" }
  ...
  context "with two dbs" do
    before { conn.config_filename = fixture_path("two_dbs.yml") }

    it "accepts a symbol for the db name" do
      with_connection(:second) { expect(conn.current_db).to eq("second") }
    ...
    it "correctly handles exceptions in threads mode" do
      begin
        conn.each_connection(threads: 2) { boom }
      rescue => e
    ...
    it "has correct all_dbs" do
      expect(conn.all_dbs).to eq(%w[default second])
    end
    ...
    context "with data partitioning" do
      after do
        %w[default second].each do |db|
          with_connection(db) do |cnn|
            begin
              cnn.execute("drop table people")
      ...
      it "partitions data correctly" do
        %w[default second].map do |db|
          with_connection(db) do |cnn|
            cnn.execute(
              "create table if not exists people(id INTEGER PRIMARY KEY AUTOINCREMENT, db)",
            )
          end
        end
        ...
        lists = []
        %w[default second].each do |db|
          with_connection(db) { |cnn| lists << Person.order(:id).to_a.map { |p| [p.id, p.db] } }
        end
    ...
    describe "reloading" do
      ...
      context "when site config is updated" do
        it "updates that connection handler" do
          ...
          conn.instance.instance_variable_set(
            :@config_filename,
            fixture_path("two_dbs_updated.yml"),
          )
          conn.reload
  ...
  describe ".current_hostname" do
    ...
    it "should return the right hostname" do
      with_connection("default") { expect(conn.current_hostname).to eq("default.localhost") }
  ...
  describe ".default_connection_handler=" do
    ...
    it "should raise the right error if attempting to assign an invalid argument" do
      expect do conn.default_connection_handler = "test" end.to raise_error(ArgumentError)
    end
