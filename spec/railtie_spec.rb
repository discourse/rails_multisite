...
describe RailsMultisite::Railtie do
  ...
  def run_initializer(multisite_fixture: nil, default_path_prefix: nil)
    ...
    app = Class.new(Rails::Application) do
      ...
      config.multisite_default_path_prefix = default_path_prefix
    end
  ...
  describe 'app.config.multisite_default_path_prefix' do
    it 'passes the value to ConnectionManagement.default_path_prefix' do
      run_initializer(multisite_fixture: 'two_dbs_path_prefix.yml', default_path_prefix: '/root')
      expect(conn.default_path_prefix).to eq('/root')
    end
  end
  ...
  describe 'ActionController::Base.config.relative_url_root' do
    ...
    it 'returns the default path prefix when multisite_default_path_prefix is set' do
      run_initializer(multisite_fixture: 'two_dbs_path_prefix.yml', default_path_prefix: '/root')
      conn.establish_connection(db: 'default')
